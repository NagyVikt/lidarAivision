...
from launch.substitutions import Command, LaunchConfiguration, PythonExpression, PathJoinSubstitution, TextSubstitution
...
def generate_launch_description():
    ...
    param_name_suffix = '_camera'  # If needed to differentiate configurations
    ...
    laser_filters_pkg = PathJoinSubstitution(
    ...
    default_rviz_config_path = os.path.join(pkg_share, 'rviz/nav2_config5.rviz')
    ...
    declare_camera_config =  DeclareLaunchArgument('json_file_path', default_value=json_file_path, description='Path to the JSON configuration file')
    # ==============================================================================
    # ==========================    LIDAR DECLARATIONS   ===========================
    # ==============================================================================
    ...
    start_lidarright_filter = Node(
        ...
        parameters=[
            PathJoinSubstitution([
                get_package_share_directory("laser_filters"),
                "examples", "angular_filter_exampleright.yaml",
            ])],
        )
    ...
    start_lidarleft_filter = Node(
        ...
        parameters=[
            PathJoinSubstitution([
                get_package_share_directory("laser_filters"),
                "examples", "angular_filter_exampleleft.yaml",
            ])],
        )
    ...
    realsense_node = Node(
        ...
        remappings=[
            # Optional: Remap topics if necessary
            ('/camera/camera/depth/color/points', '/points_raw')
        ]
    )
 ...
 # Define the pointcloud_processor node
    ...
    ld.add_action(declare_camera_config)
